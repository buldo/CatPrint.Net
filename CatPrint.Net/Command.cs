using System.Buffers.Binary;

namespace CatPrint.Net;

public class Command
{
    private const int HeaderLength = 6;

    private readonly byte _command;
    private readonly byte[] _payload;

    public Command(byte command, byte[] payload)
    {
        if (payload.Length == 0 || payload.Length > 0x100)
        {
            throw new ArgumentOutOfRangeException(nameof(payload),
                "Payload must not be empty and must be shorter than 0x100");
        }

        _command = command;
        _payload = payload;
    }

    public byte[] AsBytes()
    {
        var payloadLen = _payload.Length;

        var resultBytes = new byte[HeaderLength+payloadLen+2];

        var headerSpan = resultBytes.AsSpan(0, HeaderLength);
        headerSpan[0] = 0x51;
        headerSpan[1] = 0x78;
        headerSpan[2] = _command;
        headerSpan[3] = 0x00;
        headerSpan[4] = (byte)_payload.Length;
        headerSpan[5] = 0x00;

        _payload.AsSpan().CopyTo(resultBytes.AsSpan(HeaderLength, payloadLen));

        var msgEnd = resultBytes.AsSpan(HeaderLength+payloadLen, 2);
        msgEnd[0] = Crc8.Calculate(_payload);
        msgEnd[1] = 0xFF;

        return resultBytes;
    }
}

internal static class Crc8
{
    private static readonly byte[] Crc8Table =
    {
        ,
    };

    public static byte Calculate(byte[] data)
    {
        byte crc = 0;

        foreach (byte b in data)
        {
            crc = Crc8Table[crc ^ b];
        }

        return crc;
    }
}